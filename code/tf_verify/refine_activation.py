...
def refine_activation_with_solver_bounds(nn, self, man, element, nlb, nub, relu_groups, timeout_lp, timeout_milp, use_default_heuristic, domain, K=3, s=-2, use_milp=False, approx=True):
    ...
    if predecessor_index == 0:
        config.record_prebound_l = nn.specLB
    else:
        config.record_prebound_l = nlb[predecessor_index - 2]
    if predecessor_index == 0:
        config.record_prebound_u = nn.specUB
    else:
        config.record_prebound_u = nub[predecessor_index - 2]
    ...
    if nn.activation_counter==0:
        ...
        if config.refine_neurons==True and nn.layertypes[predecessor_index]=="FC":
            ...
            if config.debug:
                ...
                # LP时间总和config.sum_refine_time
                config.sum_refine_time += end-start
                print(f"Whole refinement of bounds time: {config.sum_refine_time:.3f}.")
