...
def sparse_heuristic_with_cutoff(nn, length, lb, ub, man, element, layerno, K=3, s=-2):
    # used.conv_count = nn.layertypes.count('Conv')
    K = config.K

    #im2col(nn.specUB.reshape(nn.input_shape[0][0], nn.input_shape[0][1], nn.input_shape[0][2]), nn.filter_size[0][0], nn.filter_size[0][1], stride, pad)
    # w = conv_kernel2matrix(nn.input_shape[1], nn.out_shapes[1], nn.filters[1])
    ...
    if config.model == 'FA':
        ...
        if len(all_vars) == 0:
            ...
            if len(all_vars) <= K:
                kact_args.append(all_vars)
            else:
                sparsed_combs = generate_sparse_cover(len(all_vars), K, s=s)
                for comb in sparsed_combs:
                    kact_args.append(tuple([all_vars[i] for i in comb]))
            ...
            # if 'Resadd' or 'Conv' in nn.layertypes:
            if 'Resadd' in nn.layertypes:
                # if None:
                all_vars_group = []
                l = len(all_vars)
                # part = l//os.cpu_count() + 1
                part = 1
                # if part == 0:
                #     part += 1
                rest_point = []
                for p in range(part):
                    all_vars_group.append(all_vars[(l // part) * p: (l // part) * (p + 1)])
                    for group in all_vars_group:
                        kact_args_group = []
                        if len(group) <= 3:
                            if len(group) <= K:
                                kact_args_group.append(group)
                            else:
                                sparsed_combs = generate_sparse_cover(len(group), K, s=s)
                                for comb in sparsed_combs:
                                    kact_args_group.append(tuple([group[i] for i in comb]))
                        else:
                            kact_args_group, rest_point = M_select_main(nn, config.relu_count, lb, ub, group,
                                                                        config.record_prebound_l,
                                                                        config.record_prebound_u, man, element, layerno)

                        if kact_args == []:
                            kact_args = kact_args_group
                        else:
                            kact_args = [*kact_args, *kact_args_group]

            else:
                kact_args, rest_point = M_select_main(nn, config.relu_count, lb, ub, all_vars, config.record_prebound_l, config.record_prebound_u, man, element, layerno)

            if len(rest_point) > 0:
                sparse_n = config.sparse_n
                vars_above_cutoff = rest_point
                ...
                # for var in rest_point:
                #     kact_args.append([var])

        num_groups = len(kact_args)

        config.effect_count += num_groups
        end = time.time()
        config.sum_args_time += end - start

        print("krelu: n", config.sparse_n,
              "split_zero", len(all_vars),
              "effect count", config.effect_count,
              "number of args", num_groups,
              "time of args", config.sum_args_time)

        config.relu_count = (config.relu_count + 1) % config.actnum
        for var in all_vars:
            kact_args.append([var])
        return kact_args
    # or used.conv_count != 0
    elif config.model == 'para':

        # if used.conv_count != 0:
        #     used.conv_count -= 1
...
def sparse_heuristic_curve(nn, length, lb, ub, is_sigm, man, element, layerno, K=config.K, s=-1):
    assert length == len(lb) == len(ub)
    if is_sigm:
    ...
    if config.model == 'FA':
        ...
        vars_above_cutoff = [i for i in vars_above_cutoff if ub[i] - lb[i] >= 0.06]
        limit = 0.02
        vars_above_cutoff = [i for i in vars_above_cutoff if lb[i] <= -limit and ub[i] >= limit]
        n_vars_after_cutoff = len(vars_above_cutoff)
        all_vars_pre = all_vars
        all_vars = vars_above_cutoff

        for var in vars_above_cutoff:
            all_vars_pre.remove(var)
        ...
        if len(all_vars) == 0:
            ...
            if 'Resadd' in nn.layertypes or 'Conv' in nn.layertypes or 'Tanh' in nn.layertypes or 'Sigmoid' in nn.layertypes:
                # if None:
            ...
            if len(rest_point) > 0:
                areas = {var: -lb[var] * ub[var] for var in rest_point}
                sparse_n = config.sparse_n
        ...
        config.sum_args_time += end - start
        for var in all_vars_pre:
            kact_args.append([var])
        print("krelu: n", config.sparse_n,
              "split_zero", len(all_vars_pre),
...
def encode_kactivation_cons(nn, man, element, offset, layerno, length, lbi, ubi, constraint_groups, need_pop, domain, activation_type, K=2, s=-1, approx=True):
    ...
    if activation_type == "ReLU":
        kact_args = sparse_heuristic_with_cutoff(nn, length, lbi, ubi, man, element, layerno, K=K, s=s)
    else:
        kact_args = sparse_heuristic_curve(nn, length, lbi, ubi, activation_type == "Sigmoid", man, element, layerno,K=K, s=s)
...
##########################################################################################################
        ...
        #         bi.append([upper_bound[i]])
        #         i = i + 1
        #
        #
        #     bi = [*bi, *bi]
        #     # b = np.mat(bi).transpose((-1,1))
        #
        #     print(bi)
        #     result = make_kactivation_obj(input_hrep, approx)
        #     for re in result:
        #         print(re)
        #         print(con2vert(np.mat(re), b))
        #
...
###########################################################################################################
    ...
    with multiprocessing.Pool(config.numproc) as pool:
    # kact_results = pool.starmap(make_kactivation_obj, input_hrep_array)
        kact_results = pool.starmap(make_kactivation_obj, zip(input_hrep_array, len(input_hrep_array) * [approx]))

    # kact_results = make_kactivation_obj(input_hrep_array[0])
        pool.close()
        pool.join()
    # end2 = time.time()
    ...
    if config.debug:
        print(f'k-activation time: {end-start:.3f}. Time for input: {end_input-start:.3f}. Time for k-activation constraints {end-end_input:.3f}.')
        # 形成约束的时间总和config.sum_cons_time
        config.sum_cons_time += end-start
        print(f'Whole time for k-activation time: {config.sum_cons_time:.3f}.')
    if domain == 'refinezono':
