...
class ERAN:
    ...
    def analyze_box(self, specLB, specUB, domain, timeout_lp, timeout_milp, use_default_heuristic,
                    ...
                    spatial_constraints=None, K=2, s=-1, timeout_final_lp=100, timeout_final_milp=100, use_milp=False,
        ...
        if config.model == 'FA':
            ...
            if 'Resadd' in nn.layertypes:
                ...
                conv_used = []
                conv_num = -1
                for type in range(len(nn.layertypes)):
                    if nn.layertypes[type] == 'Conv' and nn.layertypes[type + 1] != 'Conv':
                        conv_num += 1
                        conv_used.append(conv_num)
                    elif nn.layertypes[type] == 'Conv' and nn.layertypes[type + 1] == 'Conv':
                        conv_num += 1
                used.conv_count = len(conv_used)

                input_shapes = out_shapes = filters = biases = []

                for i in conv_used:
                    input_shapes.append(nn.input_shape[i])
                    out_shapes.append(nn.out_shapes[i])
                    filters.append(nn.filters[i])
                    biases.append(nn.biases[i])

                input_shape = []
                for i in range(0,len(filters),4):
                    input_shape.append(filters[i])

                filter = []
                for i in range(2,len(filters),4):
                    filter.append(filters[i])

                out_shape = []
                for i in range(1,len(filters),4):
                    out_shape.append(filters[i])

                bias = []
                for i in range(3,len(filters),4):
                    bias.append(filters[i])

                for conv_i in range(used.conv_count):
                    used.weight_new.append(conv_kernel2matrix(input_shape[conv_i], out_shape[conv_i], filter[conv_i]))#np.around(filter[conv_i], decimals = 4, out = None)))
                    used.bias_new.append(np.ones(used.weight_new[conv_i].shape[0]) * np.mean(bias[conv_i]))

                for my_l in nn.biases[nn.layertypes.count('Conv'):len(nn.biases)]:
                    used.bias_new.append(my_l)

                for my_l in nn.weights:
                    used.weight_new.append(my_l)

            else:
                used.conv_count = nn.layertypes.count('Conv')
                for conv_i in range(used.conv_count):
                    used.weight_new.append( conv_kernel2matrix(nn.input_shape[conv_i], nn.out_shapes[conv_i], nn.filters[conv_i]) )
                    used.bias_new.append(np.ones( used.weight_new[conv_i].shape[0] ) * np.mean(nn.biases[conv_i]) )
                for my_l in nn.weights:
                    used.weight_new.append(my_l)
                for my_l in nn.biases[used.conv_count:len(nn.biases)]:
                    used.bias_new.append(my_l)

            config.actnum = nn.layertypes.count('FC') - 1
