...
import re
import itertools
from billiard import Pool, Value, Process
...
if config.specnumber and not config.input_box and not config.output_constraints and config.dataset == 'acasxu':
...
if dataset=='acasxu':
    ...
    for box_index, box in enumerate(boxes):
        ...
        if not verified_flag and x_adex is not None:
            adex_holds, _, _, _, _, _ = eran.analyze_box(x_adex, x_adex, init_domain(domain), config.timeout_lp, config.timeout_milp, config.use_default_heuristic, constraints)
        ...
        if not verified_flag and adex_holds:
            ...
            if use_parallel_solve:
                ...
                try:
                    ...
                    res = []
                    count = 0
                    for sub_i in multi_bounds:
                        lb_sub, ub_sub = sub_i
                        res_sub = acasxu_recursive(lb_sub, ub_sub)
                        res.append(res_sub)
                        print("\n This is the {} img".format(count))
                        count += 1
                        gc.collect()

                    # res = [None]*len(multi_bounds)
                    # i = 0
                    # for sub_i in multi_bounds:
                    #     lb_sub, ub_sub = sub_i
                    #     res[i] = Process(target=acasxu_recursive, args=(lb_sub,ub_sub))
                    #     res[i].start()
                    #     res[i].close()
                    #     res[i].join()
                    #     i += 1
                    stop = time.time()
                    print("...elapsed time for k_relu: {} \n".format(stop - start))
                    total_eposilon = get_total_eposilon(config.epsilon_list)
                    print("The total epsilon of AcasXu Net is : {} \n".format(total_eposilon))
                    if all(res):
                        verified_flag = True
                        print("All of the imgs are classified correctly")
                    else:
                        verified_flag = False
                        print("There are {} imgs are vertified as wrong".format(len(res) - sum(res)))
    ...
    if config.precise_flag == 0:
        correct = 0
        img_record = []
        for idx, test in enumerate(tests):

            if correct == config.cor_num:
                break
            ...
            if perturbed_label == label:
                ...
                verified_images += 1
                print(f'The failed images are {img_record}.')
                print(end - start, "seconds")
                config.sum_time += end - start
                print(f'sum of time {config.sum_time}')
                continue

            if (not complete) or (domain not in ['deeppoly', 'deepzono']):
                print(f'img {idx} Failed')
                img_record.append(idx)
                print(f'The failed images are {img_record}.')
                print(end - start, "seconds")
                config.sum_time += end - start
                print(f'sum of time {config.sum_time}')
        ...
        # [241, 300, 320, 324, 386, 507, 553, 591]
        objection = [0,1,2,3,4,5,6,7,9,10]
        lists = np.zeros((len(objection), config.precise_num))
        i_kun = 0
        idx_data = []
        test_data = []
        for idx, test in enumerate(tests):
            if i_kun > len(objection) - 1:
                break
            if idx in objection:
                idx_data.append(idx)
        ...
        for i in range(len(objection)):
            lists[i][0] = config.pre_per

        for i in range(len(objection)):
            config.from_test = i
            config.delta = lists[i][0]
            for j in range(1, config.precise_num):
                idx = idx_data[i]
                test = test_data[i]
                ...
                if perturbed_label == label:
                    print(f'img {idx} verified {label}')
                    verified_images += 1
                    lists[i][j] = lists[i][j - 1] + 0.5 ** (j + config.conv)
                    config.delta = lists[i][j]
                ...
                print(end - start, "seconds")
        gc.collect()
    if config.precise_flag == 1:
        print(lists)
    end_01 = time.time()
    total_time = end_01 - start_01
    ...
    file_handle.write('\n sum of args is {}, sum of time is {}\n sum of groups time is {}, sum of cons time is {}, sum of refine time is {}\n analysis precision is {}/{}\n'.format(config.effect_count,config.sum_time,config.sum_args_time,config.sum_cons_time,config.sum_refine_time, verified_images, correctly_classified_images))
    file_handle.close()
    gc.collect()
else:
    target = []
    ...
    for i, test in enumerate(tests):
        ...
        print(f"progress: {1 + i - config.from_test}/{config.num_tests}, "
              ...
              f"verified: {vevimrified_images}/{correctly_classified_images}, "
