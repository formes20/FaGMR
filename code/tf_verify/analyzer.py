...
class Analyzer:
    def __init__(self, ir_list, nn, domain, timeout_lp, timeout_milp, output_constraints, use_default_heuristic, label,
                 prop, testing = False, K=config.K, s=config.s, timeout_final_lp=100, timeout_final_milp=100, use_milp=False,
    ...
    def get_abstract0(self):
        ...
        if config.domain in ["refinezono", "refinepoly"]:
            gc.collect()
        if self.testing:
    ...
    def analyze(self, terminate_on_failure=True):
        ...
        if dominant_class:
            config.epsilon_list.append([nlb[-1], nub[-1]])
